...
func _ready():
	...
	apply_character_customization(hoody_color)
...
func start_wall_run(normal: Vector3):
	if not can_wall_run:
		return
...
func apply_character_customization(color: Color):
	if hoody_mesh: # ensure it's not null (not yet instantiated)
		hoody_mesh.get_active_material(0).albedo_color = color 
